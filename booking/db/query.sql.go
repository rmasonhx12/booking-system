...
const listBookingsBeteen = `-- name: ListBookingsBeteen :many
SELECT id, start_time, end_time, email, created_at FROM booking
WHERE start_time >= $1 AND end_time <= $2
`

type ListBookingsBeteenParams struct {
	StartTime pgtype.Timestamp
	EndTime   pgtype.Timestamp
}

func (q *Queries) ListBookingsBeteen(ctx context.Context, arg ListBookingsBeteenParams) ([]Booking, error) {
	rows, err := q.db.Query(ctx, listBookingsBeteen, arg.StartTime, arg.EndTime)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
